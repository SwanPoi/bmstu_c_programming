...
START_TEST(test_start_after_end)
{
    ...
    int *dst_start = NULL, count = 0;

    int rc = key(pstart, pend, dst_start, &count);
...
START_TEST(test_start_equal_end)
{
    ...
    int rc = key(arr, arr, dst_start, &count);
...
START_TEST(test_start_is_null)
{
    ...
    int rc = key(NULL, pend, dst_start, &count);
...
START_TEST(test_end_is_null)
{
    ...
    int rc = key(pstart, NULL, dst_start, &count);
...
START_TEST(test_dst_end_is_null)
{
    ...
    int rc = key(arr, arr + sizeof(arr) / sizeof(int), dst_start, NULL);
...
START_TEST(test_empty_array)
{
    ...
    int rc = key(arr, arr + sizeof(arr) / sizeof(int), dst_start, &count);
...
START_TEST(test_mean_not_in_arr)
{
    ...
    dst_start = calloc(count, sizeof(int));

    rc = key(arr, arr + sizeof(arr) / sizeof(int), dst_start, &count);
